...
public class KeyboardInputActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textToSpeech = new TextToSpeech(KeyboardInputActivity.this, new TextToSpeech.OnInitListener() {
            @Override
        ...
        button4.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                ...
                //Toast.makeText(KeyboardInputActivity.this, "Space", Toast.LENGTH_SHORT).show();
        ...
        button5.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                ...
                if (mode == 1) {
                    ...
                    if (ch == '\0') {
                        ConvertTextToSpeech("Invalid");
                    } else {
                        ConvertTextToSpeech(s);
                    }
                    //Toast.makeText(KeyboardInputActivity.this, s + " " + Arrays.toString(word), Toast.LENGTH_SHORT).show();
                    string = string + s;
                    clearWord(word);
                    long[] a = {0, 150, 0, 150};
                    vibrator.vibrate(a, -1);
                    ...
                    mode = 1;
                    //Toast.makeText(KeyboardInputActivity.this, "Number Mode", Toast.LENGTH_SHORT).show();
                    ConvertTextToSpeech("Number");
                    clearWord(word);
                    long[] a = {0, 150, 0, 150};
                    vibrator.vibrate(a, -1);
                } else {
                    Log.v("Mask is ", String.valueOf(c));
                    Character ch = map.getBR(c);
                    String s = String.valueOf(ch);
                    if (ch == '\0') {
                        ConvertTextToSpeech("Invalid");
                    } else {
                        if (ch == '.') {
                            ConvertTextToSpeech("Period");
                        } else if (ch == ';') {
                            ConvertTextToSpeech("Semi colon");
                        } else if (ch == ',') {
                            ConvertTextToSpeech("Comma");
                        } else if (ch == '?') {
                            ConvertTextToSpeech("Question Mark");
                        } else if (ch == '!') {
                            ConvertTextToSpeech("Exclamation Mark");
                        } else {
                            ConvertTextToSpeech(s);
                        }
                    }
